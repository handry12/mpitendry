...
4. **Build production**
   ...
   npm run preview
   ```
   En production, exposer le serveur (`npm run server`) et configurer le reverse proxy pour que `/api` pointe vers ce serveur, ou servir les fichiers `dist` depuis le même serveur Express.
